...
当前源码基线为 `v0.8.0`。账户筹码、PostgreSQL 持久化、管理治理和单实例生产部署链路均已落地，项目已具备可供熟人联机使用的完整功能：
...
- 牌局记录与回放：列出本人参与过的每一手（可往前翻页），含公共牌、各家筹码变化与按街的动作过程；任一手都能像录像一样逐步回放，只显示本人的底牌与摊牌时亮出的牌。另有结算历史、追加式筹码账本、基础设置、轻量牌桌动画，以及下注、All in、过牌和弃牌的独立牌桌音效。
