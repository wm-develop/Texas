...
当前源码基线为 `v1.0.3`。账户筹码、PostgreSQL 持久化、管理治理和单实例生产部署链路均已落地，项目已具备可供熟人联机使用的完整功能：
...
- 仅在每手结算后，弃牌玩家及因其他玩家全部弃牌而获胜的玩家可主动展示底牌；局中弃牌玩家可向任意本手发过底牌的玩家（包括其他已弃牌玩家）发起私下看牌申请；被申请者可只拒这一次、本手不再接受此人或本手不再接受任何人，同意过一次后本手不能再向其申请。每位玩家可在设置里关闭「允许别人申请换座 / 私下看我的牌」（房间内有效，离开即重置）；拒绝换位可选「不再接受此人」。申请被拒的提示只有申请者看到。
...
- 观战位（OB）：房间成员可不占座位地观战，最多 10 人；付看牌费（默认 10 个大盲，房主可设 0～100，0 为免费）后可见本手所有参与者的手牌，费用在发牌前收取并平分给本手玩家。房主可分别关闭观战者的语音、文字与表情。手牌按接收者裁剪，普通玩家永远拿不到别人的牌。
- 抽水：管理员可按房间设置每手按底池比例抽水（不超过 10%，可设封顶）以及发出翻牌后的固定加抽（不超过一个大盲），默认不抽；抽走的娱乐筹码进入管理员钱包，规则变更在房间文字聊天里公告，每手抽了多少写在结算里。管理员账号不能创建或加入牌桌。后台可按房间查看累计抽水并做本机换算，服务端不记录任何金额。
- WebSocket 事件序列、请求幂等、断线补发和私人快照安全恢复。
- 进行中的牌局跨进程重启恢复：引擎状态写入数据库，崩溃、OOM 或宿主机重启后那一手照原样继续，而不是作废。状态自相矛盾或房间盲注被改过时放弃恢复，退回作废的旧行为。
- 登录态持久化（移动端）：刷新令牌留在设备上，被系统回收后回来直接落回原来那一桌，不必重新登录。Web 端刻意不持久化，因为浏览器常常是公用的。
- 牌桌文字聊天、收起状态未读提醒、快捷语、表情、本地屏蔽和服务端禁言；点击其他玩家头像可发送带全桌动画与音效的赞赏或嘲讽。
- TRTC 自由麦语音、语音成员、开麦/说话状态、单人静音和播放音量。
- 牌局记录与回放：列出本人参与过的每一手（可往前翻页），含公共牌、各家筹码变化与按街的动作过程；任一手都能像录像一样逐步回放，只显示本人的底牌与摊牌时亮出的牌。管理员开通的账号还能在回放里发起 AI 复盘：由兼容 OpenAI 接口的大模型（默认 DeepSeek）逐步点评本人的决策并结合对手的打法倾向，局面数字由服务端算好，发给模型的数据不含昵称与没亮过的牌。另有结算历史、追加式筹码账本、基础设置、轻量牌桌动画，以及下注、All in、过牌和弃牌的独立牌桌音效。
...
- **每个版本改了什么、升级时要注意什么**，见发布说明：[v0.6.0](docs/releases/v0.6.0.md) · [v0.5.0](docs/releases/v0.5.0.md) · [v0.4.3](docs/releases/v0.4.3.md) · [v0.4.2](docs/releases/v0.4.2.md) · [v0.3.0](docs/releases/v0.3.0.md) · [v0.2.1](docs/releases/v0.2.1.md) · [v0.2.0](docs/releases/v0.2.0.md) · [v0.1.1](docs/releases/v0.1.1.md) · [v0.1.0](docs/releases/v0.1.0.md)。
