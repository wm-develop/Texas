...
- 弃牌玩家及因其他玩家全部弃牌而获胜的玩家可主动展示底牌；每手结算后进入 10 秒自动准备倒计时，玩家可取消自动准备。
...
- 一次性首管理员引导、服务器端权限校验、账号批量治理、在线/所在房间状态、强制离房、用户名与筹码调整、密码重置、新用户注册开关和管理审计。
...
管理员登录后可从右上角齿轮进入“服务器管理”，查看账号在线状态、筹码及所在房间，修改用户名和钱包筹码，将玩家请出房间，批量创建/停用/恢复/删除账号、重置密码，以及开启或关闭新用户注册。删除为保留历史数据的软删除；正在牌桌中的账号必须先离桌才能调整筹码、停用或删除。所有已登录用户均可点击大厅右上角自己的登录用户名进入个人信息页，修改登录用户名和密码。
