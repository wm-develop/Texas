...
阶段 3 正在进行。账户娱乐筹码余额、无支付虚拟充值、最近筹码流水、房主自定义盲注/最大带入、玩家自主带入、手间补码、输光自动补码和离桌返还已经形成完整纵向链路；盲注最低为 10/20，普通下注和加注统一使用小盲整数倍。PostgreSQL 已覆盖账户/会话、钱包流水、房间成员、牌局行动与历史、结算账本和文字聊天，并可通过配置切换运行时；关键带入、补码、结算和返还采用事务、行锁与请求幂等。下一步是使用真实 PostgreSQL 完成集成验收，再进入 Redis 多实例恢复、管理治理和稳定性验收。执行顺序见 [阶段 3 计划](docs/PHASE_3_PLAN.md)。

> 游戏服务默认仍使用进程内仓储，方便无数据库开发。设置 `STORAGE_BACKEND=postgres` 并完成迁移后会启用 PostgreSQL 持久化。真实数据库集成测试尚未在本机执行，因此当前仍只适合本地及封闭联机测试。
