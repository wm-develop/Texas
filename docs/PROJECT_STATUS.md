...
- 观战位：房间成员可不占座位地观战，最多 10 人。付看牌费（默认 10 个大盲，房主可设 0～100，0 为免费）后能看到本手所有参与者的手牌，费用在**发牌前**收取并平分给即将参与的上桌玩家（余数从庄位起顺时针逐枚）；余额不足则本手不收费也不给看牌。切换在手间立即生效，牌局中只记意向、手结束后应用。房主可分别关闭观战者的语音/文字/表情。右上角「X/10（OB: Y）」点开房间名单。手牌按接收者裁剪，普通玩家永远拿不到别人的牌。
