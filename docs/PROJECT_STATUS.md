...
- HarmonyOS 开麦后听不到远端（真机复现并定位）：真因是**牌局提示音**，不是 TRTC 角色切换。鸿蒙音频焦点是应用级的，`audioplayers` 的 `FocusManager` 每次播放前调用 `setAudioSessionScene(AUDIO_SESSION_SCENE_MEDIA)` 并以 `CONCURRENCY_DEFAULT` 激活音频会话，该模式独占物理输出通道并压制应用内其他音频流，于是每有玩家动作播一声提示音，系统音量类型就被改成媒体音量（音量条话筒变喇叭）、TRTC 通话流被压制。用户实测无动作时静置一分钟音量类型始终是话筒，独立佐证了这一点。修复：HarmonyOS 语音进行中改由 TRTC 的 `TXAudioEffectManager.startPlayMusic`（`publish: false`）播放提示音，声音走通话流，不产生第二个音频会话，语音与提示音并存；提示音字节先落盘到临时目录供 RTC 按路径播放。该平台的全局并发策略同时设为 `CONCURRENCY_MIX_WITH_OTHERS` 作为兜底。RTC 通道任何一步不可用都安静跳过这一声，绝不回退到普通音频插件——回退正是会掐掉远端语音的路径。Android/Windows 行为不变。**待真机验证。**
