...
- 每手结束后 10 秒自动准备，允许主动取消；筹码为零时要求补码。
...
- 客户端版本仍写为 `0.1.0+1`，与源码标签 `v0.1.1` 不一致；建立下一版本发布流程时应统一版本号和构建号。
