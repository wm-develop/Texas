...
- HarmonyOS 开麦后听不到远端（真机复现并定位）：真因是**牌局提示音**，不是 TRTC 角色切换。鸿蒙音频焦点是应用级的，`audioplayers` 的 `FocusManager` 每次播放前调用 `setAudioSessionScene(AUDIO_SESSION_SCENE_MEDIA)` 并以 `CONCURRENCY_DEFAULT` 激活音频会话，该模式独占物理输出通道并压制应用内其他音频流，于是每有玩家动作播一声提示音，系统音量类型就被改成媒体音量（音量条话筒变喇叭）、TRTC 通话流被压制。用户实测无动作时静置一分钟音量类型始终是话筒，独立佐证了这一点。修复经过两轮：先让位（语音中不出声），再改走 TRTC 音频引擎——后者真机实测音量偏小且有延迟断续，因为那条链路是给背景音乐用的，不是给 UI 音效用的。当前采用第三种：仍走普通音频插件，但把并发模式从 `CONCURRENCY_DEFAULT`（独占输出并压制其他音频）改为 `CONCURRENCY_MIX_WITH_OTHERS`。关键在于必须设到播放器实例上——鸿蒙插件的全局 `setAudioContext` 只更新新建播放器的默认值，而决定并发模式的是 `FocusManager` 读到的 `player.context`，只设全局等于没设。三种方式保留为 `HarmonyVoiceSoundStrategy`，改一个默认值即可切换。Android/Windows 行为不变。**待真机验证。**
