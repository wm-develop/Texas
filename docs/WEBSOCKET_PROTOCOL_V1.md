...
房主离桌后，服务端按成员加入时间、再按座位号将 `ownerUserId` 转移给最早加入的剩余成员。房主若是弃牌后中途离开，他要到本手结算才真正移出房间，房主移交也随之推迟到结算之后。主动亮牌只允许在本手结算后由已弃牌玩家、或因其他玩家全部弃牌而获胜的玩家使用，公开状态持续到下一手开始。每手结算后服务端设置 10 秒 `autoReadyDeadline`（发两次结算为 15 秒，客户端分两块牌面先后展示需要额外时间）；短暂断线不等同于主动取消，客户端发送 `table.ready.set {"ready": false}` 才取消本轮自动准备，再发送 `ready: true` 可恢复。
...
**进行中的手会跨重启恢复**（0.5.0 起，需接入 PostgreSQL）：每次引擎推进后，牌桌的完整状态（含底牌、未发的牌堆、底池、行动位、加时卡、观战者看牌权）写入 `table_states`，手结束即删除。进程崩溃、OOM 或宿主机重启后，第一个回到该房间的人触发恢复，那一手照原样继续，行动倒计时重新安排。

恢复是**尽力而为**：状态版本对不上、内容自相矛盾（牌重复、筹码为负、轮到空座位）、或停机期间房间盲注被改过，一律放弃恢复并清除该记录，退回旧的「本手作废」行为——拿一个不确定的状态继续发牌，比作废一手严重得多。未接数据库的部署行为不变。

客户端仍以 `session.authenticated` 的 `serverInstanceId` 识别服务端重启，但**不能就此判定作废**：要等重连后的第一条快照，`handId` 与重启前相同说明那手已被恢复，换了手号才是真的作废。仅凭「重连后 `handId` 变了」也不能单独下结论——玩家掉线期间那一手可能已经正常结算并进入下一手，所以两个条件缺一不可。
...
| `leave_pending` | 本人弃牌后中途离开，那一手还没结算：成员记录要留到结算后才移出，这期间不能创建或加入其他房间（回到原房间则视为撤销离开）。仅 HTTP 创建/加入房间接口返回，状态码 409 |
