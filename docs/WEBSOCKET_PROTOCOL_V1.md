...
房主离桌后，服务端按成员加入时间、再按座位号将 `ownerUserId` 转移给最早加入的剩余成员。主动亮牌只允许在本手结算后由已弃牌玩家、或因其他玩家全部弃牌而获胜的玩家使用，公开状态持续到下一手开始。每手结算后服务端设置 10 秒 `autoReadyDeadline`；短暂断线不等同于主动取消，客户端发送 `table.ready.set {"ready": false}` 才取消本轮自动准备，再发送 `ready: true` 可恢复。
