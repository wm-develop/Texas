...
hand_actions          玩家行动
hand_results          结算结果
wallet_accounts       娱乐筹码账户
wallet_entries        筹码流水
table_chat_messages   牌桌文字消息
chat_mutes            禁言与屏蔽关系
user_reports          用户举报
voice_sessions        语音频道加入与退出元数据
audit_events          审计事件
client_versions       客户端版本策略
```
...
- 数据库备份和恢复演练。
- 弱网、压力和 RTC 稳定性测试。
- Web 部署、Windows 安装包、Android 包和 HarmonyOS 包。
...
1. 创建阶段 0 技术验证任务清单。
2. 记录 Flutter OH 实际版本，锁定 DevEco、Android 和 Go 工具链版本。
3. 验证腾讯云 TRTC 在 Web、Windows、Android 和 HarmonyOS 的接入路径。
4. 输出德州扑克规则规格和 WebSocket 协议 v1。
5. 制作 10 人牌桌低保真原型和响应式布局规范。
6. 阶段 0 评审通过后，再进入正式功能开发。
