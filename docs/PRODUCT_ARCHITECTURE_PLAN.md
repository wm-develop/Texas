...
- 隐私说明与账号自助注销已实现，规则见[隐私说明](PRIVACY_NOTICE.md)。
...
- 断线重连后可拉取少量最近消息。
- 管理员持久化文字禁言和可查询的审计记录。玩家举报经评估后不做：熟人私人牌局下它解决不了实际问题。
...
- 牌局事件及娱乐筹码流水审计。
- 客户端最低版本和公告配置（尚未实现）。
- 健康/就绪检查、结构化错误日志、令牌保护的 `/metrics`、健康巡检与告警均已实现，见[运行保障指南](OPERATIONS_GUIDE.md)。
...
- 生产形态是单 Go 实例。Redis、多实例租约和运行中牌桌故障接管已评估并决定暂缓，理由、若要实施的约束与重新评估的触发条件见 [ADR-002](decisions/ADR-002-MULTI-INSTANCE.md)。
...
- 首次开启语音前明确说明麦克风用途。
- 提供静音和屏蔽入口。
- 服务端以审计事件记录语音加入/退出与禁言等元数据，不记录音频内容。
...
语音加入/退出元数据复用 `audit_events` 表记录。客户端版本策略尚未建表；举报经评估后不做。
...
- 基础聊天治理、管理审计查询与语音进出元数据已完成；玩家举报经评估后不做。
- 日志、监控和告警已完成。
- 隐私说明和账号注销流程已完成。
- 灰度版本控制（未做）。
- 数据库备份和恢复演练已完成。
- 弱网、压力和 RTC 稳定性测试：工具与清单已就绪，尚未实跑，见[验收指南](ACCEPTANCE_GUIDE.md)。
- Web/Windows/Android/HarmonyOS 构建和部署流程、Android 正式签名与 CI 均已完成。

退出标准：系统持续运行 24 小时无牌局状态错误；游戏、文字聊天和语音中任一服务故障时，其余关键功能可以按设计降级或恢复。

该阶段的任务拆分与完成情况已并入[开发历程](DEVELOPMENT_HISTORY.md)，剩余事项见[项目现状](PROJECT_STATUS.md)的推荐下一步。
...
阶段 0～2 和阶段 3 单实例主链均已实施。后续不再按最初评审步骤推进，接手者应从[项目现状](PROJECT_STATUS.md)的风险与推荐下一步、[项目交接文档](PROJECT_HANDOVER.md)和[开发历程](DEVELOPMENT_HISTORY.md)开始。
