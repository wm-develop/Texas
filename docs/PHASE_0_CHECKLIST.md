...
> 历史完成记录。部分早期表述已被后续实现替代；当前状态以[项目现状](PROJECT_STATUS.md)为准。
...
- [x] 单桌人数调整为 2～10 人。
- [x] 明确仅供熟人使用，不公开传播。
- [x] 明确注册账号和封闭传播边界；最终实现采用管理员注册开关和受管账号，不使用邀请码。
- [x] 语音选用腾讯云 TRTC。
- [x] 语音故障与牌局连接隔离。
...
- [x] 四端能够连接同一个 Go 服务端（后续阶段及构建/实机验证完成）。
- [x] 四端能够发送和接收牌桌文字消息（后续阶段完成）。
