...
**最低客户端版本不在这里配置。** 它存在数据库里，在客户端的「服务器管理」页面点「最低客户端版本」即可调整，立即生效，不需要改环境变量、也不需要重建容器——只更新客户端的那些次发布因此完全不用登服务器。取值就是新客户端 `pubspec.yaml` 里 `+` 后面那个数（0.2.1 → 2001），填 0 表示不限制。**调高它之前先把新客户端发出去**，否则还没更新的人会立刻被挡在门外。
