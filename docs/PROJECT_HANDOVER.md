...
> 交接基线：`main` 分支，最近源码标签 `v0.2.1`，2026-09-02。
>
...
- 抽水只是娱乐筹码从底池流向管理员钱包；服务端不存储任何金额，客户端的折算只在本机计算。改动抽水相关代码时记住三件事：**守恒式是「输赢之和 + 抽水 = 0」**（引擎 `settleHand`、`history.validate`、`bankroll.ApplySettlement` 三处各自校验，改一处要看另外两处）；**规则只在两手之间进引擎**（`applyRakeForNextHandLocked`），不要让进行中的一手中途换规则；**`holdem.Table.rake` 与 `runtime.rakeRecipientID` 都在崩溃恢复状态里**，新增相关字段要同步 `State()`/`RestoreTable()` 与 `runtimeState`。设置接口用 POST 而不是 PUT：Web 端 CORS 只放行 GET/POST。抽水在各池之间**按比例分摊**（`splitRake`），不要改回从主池起依次扣——独立审查里两位审查者都指出那会让短码全下的赢家替边池付全部抽水、主池被扣到 0。改房间抽水只能走 `SaveRake`，不要用整体 `Save`。`splitRake` 与 `RakeConfig.Amount` 里的乘法不要改回两个 int64 直接相乘：建房不限盲注与带入，百亿级底池会溢出，而守恒校验发现不了。
...
- 默认分支：`main`。
- 已有标签：`v0.1.0`、`v0.1.1`、`v0.2.0`、`v0.2.1`。
...
- `internal/review`：AI 复盘。`facts.go` 在服务端算好局面数字与对手倾向，`prompt.go` 是提示词（改提示词要同时升 `PromptVersion`，旧结果按版本缓存，不会被新版本覆盖），`client.go` 是兼容 OpenAI 接口的客户端，`service.go` 是排队、额度（全局与按人的每日次数、同时进行上限、全服 token 预算）、多个工作协程、按 DeepSeek 错误码的自动重试与余额不足/密钥失效时的暂停。喂给模型的数据只能来自裁剪后的牌谱，`TestPromptCarriesNoIdentityOrHiddenCards` 守着不泄露名字、账号与没亮过的牌。调提示词时用 `internal/game/tablemanager/review_live_test.go`：它用真实牌桌管理层让几种风格的机器人打两百多手，挑本人八类有代表性的手交给真实模型，把提示词与原始输出写进 `REVIEW_LIVE_OUT` 目录；只从 `REVIEW_LIVE_ENV` 指向的 env 文件读 `REVIEW_*` 几项，默认跳过，运行会产生费用。
...
- **给 `holdem.Table` 或 `tablemanager.runtime` 新增字段时，要同步崩溃恢复的状态结构。**牌局状态会被序列化进 `table_states` 供进程重启后恢复（见协议 6.8）：引擎侧改 `State()` 与 `RestoreTable()`，牌桌管理层改 `runtimeState`/`managerHandState`。漏掉一个字段，恢复出来的牌局看着正常，只有那个字段悄悄回到零值。引擎侧有一条按字段数量比对的守卫测试会拦住漏改，管理层侧没有对应的反射守卫，只能靠这条约定和代码评审。
- **凡是要拿去同步引擎的成员表，必须在持有牌桌锁期间取到。** `syncMembers` 看到「成员表里有、引擎里没有」就会把人加进引擎；如果成员表是锁外取的，等锁的这几十毫秒里有人离开，他就会被当成新玩家加回来——一个数据库里不存在的幽灵座位，此后每一手结算都因为筹码对不上账而失败。线上真发生过：自动准备倒计时到点、服务端替每个人点准备的同一瞬间有人离开，一晚上坏了两个房间、66 手输赢没进账户。`SetReady` 现在在锁内写房间服务并用它返回的成员表同步，`Join` 与 `Snapshot` 都在拿到锁后才取成员表（`Snapshot` 是广播时每个客户端都走的路径，窗口最宽）。新增走 `syncMembers` 的路径必须遵守同一条规则，`beforeLockForTest` 钩子可用来写交错测试。
- **结算落盘失败必须可见，且不能开下一手。** 落盘失败此前只有内部错误才记日志，业务错误码一声不响；行动者收到一条小提示，其他人连快照都收不到，看起来就是「弃牌没反应」；引擎照常发下一手，账户一分不记。现在失败写 Error 日志、动作被拒后也广播快照，`startHandIfReadyLocked` 在上一手没入账时先补做、不开局。
- **桌上筹码记在 `room_members.table_chips` 上，成员记录不能早于筹码结清被删除。** 内存版的钱包仓储把桌上余额另存一处，删成员不影响它，所以「先删成员、结算后再退还」这种顺序错误在单元测试里全绿，到了 PostgreSQL 上筹码就连同那一行一起消失，而且不留任何流水。线上因此真的丢过筹码（房主弃牌后中途离开，8530 没退回；同一房间第二次离开又因为退还编号重复被跳过，再丢 10240）。现在的规则是：弃牌中途离开只登记「待离开」，成员记录留到本手结算，由 `finishPendingLeavesLocked` 退还后再移出；离桌退还的幂等编号带上入座时间（取微秒，PostgreSQL 的 `timestamptz` 只到微秒）。凡是动到「离开 / 移出 / 退还」顺序的改动，必须跑 `internal/room` 里的 PostgreSQL 集成用例。
- **牌谱里的玩家不能只取结算后的引擎座位。** 引擎在结算的同时就移除了「本手结束后离开」的玩家；他若下过注而牌谱里没有他，整手输赢对不上账，牌谱写入失败，结算跟着报错。离开时要把他的座位与底牌记进 `runtime.departedSeats`（随牌桌状态持久化）。
- **`Future.timeout` 不会取消底层操作。** 客户端的会话恢复给三个串行请求加了 8 秒总超时，但超时只是放弃等待：那个请求可能十几秒后才成功返回，继续执行后面的代码。因此凡是给一段带副作用的异步流程加超时，都要配一个代次标记，让迟到的结果知道自己已经作废——否则界面会在玩家已经回到登录页之后自己跳走，甚至把他手动登录的账号覆盖掉。同理，标记「正在进行中」的布尔量必须由真正执行的那个 future 释放，不能由超时的等待方释放，否则会放进第二个并发流程。
- **刷新令牌是一次性的。** 服务端 `Refresh` 会先删掉旧会话再建新的（`account/service.go`），所以旧令牌在响应返回的那一刻就已作废。客户端拿到轮换后的新令牌必须**等它写入落盘再继续**，不能「发射后不管」：写入未完成时进程被系统回收，存储里就是一个必然被拒的令牌，而「被系统回收」正是持久化要应对的场景。
...
- **弹窗里的可滚动内容区一律用 `DialogScrollArea`，高度按屏幕真实可用空间算，不要写「屏幕高度 × 比例」。** 上游 Flutter 的 AlertDialog 会把内容压到标题、按钮之外的剩余空间，限高写错了也会被悄悄修正；OpenHarmony 那版没有这一步，写多少就是多少。设置弹窗曾按 70% 限高：鸿蒙手机屏幕约 360 高，70% 是 252，弹窗里实际只能看见约 150，滚动范围按 252 算，**最后一项永远落在被裁掉的那一截里**——普通房主滚到「自动加入语音」就到底，管理员多一项反而能看到「房间管理」，但「打开服务器管理」又成了看不见的那一项。安卓、鸿蒙平板走上游逻辑一直正常，本机测试也永远复现不了，因为测试用的就是上游 Flutter。
