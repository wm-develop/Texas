...
- 抽水只是娱乐筹码从底池流向管理员钱包；服务端不存储任何金额，客户端的折算只在本机计算。改动抽水相关代码时记住三件事：**守恒式是「输赢之和 + 抽水 = 0」**（引擎 `settleHand`、`history.validate`、`bankroll.ApplySettlement` 三处各自校验，改一处要看另外两处）；**规则只在两手之间进引擎**（`applyRakeForNextHandLocked`），不要让进行中的一手中途换规则；**`holdem.Table.rake` 与 `runtime.rakeRecipientID` 都在崩溃恢复状态里**，新增相关字段要同步 `State()`/`RestoreTable()` 与 `runtimeState`。设置接口用 POST 而不是 PUT：Web 端 CORS 只放行 GET/POST。抽水在各池之间**按比例分摊**（`splitRake`），不要改回从主池起依次扣——独立审查里两位审查者都指出那会让短码全下的赢家替边池付全部抽水、主池被扣到 0。改房间抽水只能走 `SaveRake`，不要用整体 `Save`。
