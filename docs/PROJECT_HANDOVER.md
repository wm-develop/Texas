...
1. PostgreSQL 定时备份、异机保存和恢复演练。
2. 账本/结算对账任务、结构化指标和告警。
3. 登录、注册、HTTP、WebSocket、聊天和 TRTC 凭证分层限流。
4. 举报、账号注销、隐私提示和管理审计查询。
5. GitHub Actions 和密钥扫描。

### P2：再进入多实例

先写 ADR，明确 Redis 中保存什么、牌桌单写者租约、实例崩溃后的状态恢复、客户端路由和 Redis 故障降级。多实例上线前必须证明同一牌桌不会双写且筹码守恒。当前架构不能通过简单启动第二个容器获得安全的横向扩展。
